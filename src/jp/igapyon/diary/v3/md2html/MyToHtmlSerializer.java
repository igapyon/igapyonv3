...
public class MyToHtmlSerializer extends ToHtmlSerializer {
	...
	public void visit(HeaderNode node) {
		...
		if (tag.equals("h2")) {
			printer.print('<').print(tag).print(" class=\"alert alert-warning\"").print('>');
			visitChildren(node);
			printer.print('<').print('/').print(tag).print('>');
		} else if (tag.equals("h3")) {
			printer.print('<').print(tag).print(" class=\"bg-success\"").print('>');
			visitChildren(node);
			printer.print('<').print('/').print(tag).print('>');
		} else if (tag.equals("h4")) {
			printer.print('<').print(tag).print(" class=\"bg-info\"").print('>');
			visitChildren(node);
			printer.print('<').print('/').print(tag).print('>');
		} else {
			printTag(node, "h" + node.getLevel());
